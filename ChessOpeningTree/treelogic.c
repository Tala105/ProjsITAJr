...
board createBoard(){
	board b;
	strcpy(b.state, "RNBQKBNRPPPPPPPP                                pppppppprnbqkbnr0@");
	return b;
}

int kingMove(int xi, int yi, int xf, int yf, board b){
	if(abs(xi-xf)>1 || abs(yi-yf)>1) return 0;
	return 1;
}

int promote(int xi, int yi, int x, int y, board *b, void (*promotion_cb)(char *)){
	char np;
	promotion_cb(&np);
	np = tolower(np);
...
int knightMove(int xi, int yi, int xf, int yf, board b){
	...
	if(abs(xf-xi)+abs(yf-yi)==3) return 1;
	return 0;
}
...
int validateMove(board *b, int* move, void (*promotion_cb)(char *)){
	...
	b->state[65] = '@';
	if(tolower(piece) == 'k') return kingMove(move[0], move[1], move[2], move[3], *b);
	...
	if(tolower(piece) == 'r') return rookMove(move[0], move[1], move[2], move[3], *b);
	if(tolower(piece) == 'q') return queenMove(move[0], move[1], move[2], move[3], *b);
	return 0;
}

int validateState(board b, void (*promotion_cb)(char *)){
...
board makeMove(board *b, int *move, void (*promotion_cb)(char *)){
	board *n = b, bc = *b;
	if (validateMove(b, move, promotion_cb)){
		n->state[8*move[2]+move[3]] = n->state[8*move[0]+move[1]];
		n->state[8*move[0]+move[1]] = ' ';
		n->state[64] = 48 + (n->state[64]==48);
		if(!validateState(*n, promotion_cb)) return bc;
		return *n;
	}
	return bc;
}
...
char *deepLines(board b, int max_depth, int cur_depth, sqlite3 *db){
	...
	do{
		...
		if(rc == SQLITE_ROW){
			...
			char *cr = deepLines(n, max_depth, cur_depth+1, db);
			if(cr[0]!='\0'){
				for(int i=0; i<=cur_depth; i++)
					strcat(result, "   ");
				strcat(strcat(result, "↳"), cr);
			}
			free(cr);
		}
	}while(rc == SQLITE_ROW);
